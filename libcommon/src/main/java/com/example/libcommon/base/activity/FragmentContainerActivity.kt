package com.example.libcommon.base.activity
...
import com.example.libcore.fragment.IPage
...
open class FragmentContainerActivity : AppCompatActivity() {
    ...
    private fun addFragment() {
        if (fragmentPath.isNullOrEmpty()) {
            Log.i(TAG, "fragment path is null, return")
            finish()
            return
        }
        ...
        if (fragment == null) {
            ...
            if (fragment == null) {
                Log.e(TAG, "no fragment from " + fragmentPath)
                finish()
                return
            }
